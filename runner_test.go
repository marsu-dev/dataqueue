...
func TestStartSimpleConsumer(t *testing.T) {
	...
	tests := []struct {
		...
		{"CountRunnerStop", args{ctx, &CountRunner{action: ActionStop}, 4}, want{false, 4, 0}},
		{"CountRunnerStopAll", args{ctx, &CountRunner{action: ActionStopAll}, 4}, want{false, 1, 0}},
		{"CountRunnerSendAndStop", args{ctx, &CountRunner{action: ActionSendAndStop}, 4}, want{false, 4, 4}},
		{"CountRunnerSendAndStopAll", args{ctx, &CountRunner{action: ActionSendAndStopAll}, 4}, want{false, 1, 1}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := StartSimpleConsumer(tt.args.ctx, tt.args.runner, tt.args.nbP); (err != nil) != tt.want.err {
